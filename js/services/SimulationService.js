...
export class SimulationService {
    ...
    _applyEventEffects(outcome) {
        outcome.effects.forEach(effect => {
            ...
            switch (effect.type) {
                case 'credits': 
                    state.player.credits += effect.value; 
                    this._logTransaction('event', effect.value, 'Received credits from event');
                    break;
                ...
                case 'add_debt': 
                    state.player.debt += effect.value;
                    this._logTransaction('loan', effect.value, 'Incurred debt from event');
                    break;
                case 'add_cargo':
                ...
                case 'sell_random_cargo_premium':
                    ...
                    if (toSell.length > 0) {
                        ...
                        const saleValue = state.market.galacticAverages[id] * effect.value * item.quantity;
                        state.player.credits += saleValue;
                        this._logTransaction('trade', saleValue, 'Emergency supply drop sale');
                        item.quantity = 0;
                    }
                    break;
                ...
                case 'resolve_space_race':
                    ...
                    if (Math.random() < winChance) {
                        ...
                        this._logTransaction('event', wager, 'Won space race wager');
                        outcome.description = `Your Class ${ship.class} ship wins! You gain <span class="hl-green">${formatCredits(wager)}</span>.`;
                    } else {
                        state.player.credits -= wager;
                        this._logTransaction('event', -wager, 'Lost space race wager');
                ...
                case 'resolve_adrift_passenger':
                    ...
                    if (calculateInventoryUsed(inventory) + 40 <= ship.cargoCapacity) {
                        ...
                        this._logTransaction('event', paid, 'Passenger paid off debt');
                        ...
                        this._logTransaction('event', credits, 'Passenger payment');
    ...
    showIntroSequence() {
        ...
        You've acquired a used C-Class freighter, the <span class="hl">${starterShip.name}</span>, with <span class="hl-blue">⌬ ${CONFIG.STARTING_CREDITS.toLocaleString()}</span> in starting capital.
        <div class="my-3 border-t-2 border-cyan-600/40"></div>
